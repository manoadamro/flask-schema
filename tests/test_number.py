...
class NumberTest(unittest.TestCase):
    ...
    def test_min_only(self):
        prop = flask_schema.types.Number(min_value=0)
    ...
    def test_max_only(self):
        prop = flask_schema.types.Number(max_value=10)
        self.assertEqual(prop(5), 5)
    ...
    def test_min_and_max(self):
        prop = flask_schema.types.Number(min_value=0, max_value=10)
        self.assertEqual(prop(5), 5)
    ...
    def test_no_range(self):
        prop = flask_schema.types.Number()
    ...
    def test_nullable_allows_null(self):
        prop = flask_schema.types.Number(nullable=True)
        self.assertIsNone(prop(None))

    def test_nullable_raises_error(self):
        prop = flask_schema.types.Number(nullable=False)
        self.assertRaises(flask_schema.errors.SchemaValidationError, prop, None)

    def test_default_is_none(self):
        prop = flask_schema.types.Number(default=None)
        self.assertIsNone(prop(None))

    def test_default_value(self):
        prop = flask_schema.types.Number(default=12)
    ...
    def test_default_callable(self):
        prop = flask_schema.types.Number(default=lambda: 12)
        self.assertEqual(prop(None), 12)

    def test_wrong_type(self):
        prop = flask_schema.types.Number(callback=None)
        self.assertRaises(flask_schema.errors.SchemaValidationError, prop, "nope")

    def test_callback(self):
        prop = flask_schema.types.Number(callback=lambda v: v * 2)
